...
It helps prevent prop drilling which can be cumbersome for larger applications.
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
Store is the application state. Reducers take data and filter it into the state, actions send data to the reducer.
- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
Application state is the data that the whole application needs, component state is the state that only the local component relies on.
- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
  Thunk uses action creators to return a function, it allows us to delay an action or dispatch an action when conditions are met.
- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!
  I don't have a favorite, they all seem to do the same thing. Context seems to take less setup though.
